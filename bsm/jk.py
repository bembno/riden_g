...
def parse_basic_info(data: bytearray):
    if len(data) < 10:
        return {}
    pack_voltage = struct.unpack_from("<H", data, 0)[0] / 100
    pack_current = struct.unpack_from("<h", data, 2)[0] / 100
    num_cells = data[4]
    num_temperatures = data[5]
    balance_status = data[6]
    return {
        "Pack Voltage (V)": pack_voltage,
        "Pack Current (A)": pack_current,
